...
import com.academiadev.dto.CourseCatalogDTO;
import com.academiadev.dto.CourseExportDTO;
import com.academiadev.dto.EnrollmentDTO;
import com.academiadev.dto.EnrollmentExportDTO;
...
import com.academiadev.model.PremiumPlan;
import com.academiadev.model.Student;
import com.academiadev.model.SubscriptionPlan;
import com.academiadev.model.User;
import com.academiadev.service.CourseService;
import com.academiadev.service.EnrollmentService;
import com.academiadev.service.SupportTicketService;
import com.academiadev.service.UserService;
import com.academiadev.util.GenericCsvExporter;
import com.academiadev.service.ReportService;
...
public class CliController {
    ...
    public void start() {
        ...
        while (true) {
            currentUser = null;
            boolean shouldExit = showWelcomeMenu();
            if (shouldExit) {
            ...
            if (currentUser != null) {
                if (currentUser instanceof com.academiadev.model.Admin) {
                    showAdminMenu();
                } else if (currentUser instanceof com.academiadev.model.Student) {
                    showStudentMenu();
                }
            }
        }
    ...
    private boolean showWelcomeMenu() {
        while (currentUser == null) {
            int opcao = view.mostrarMenuBoasVindas();
    ...
    private void abrirTicketSemLogin() {
        String email = view.lerEmailParaTicket();
        if (email.isEmpty()) {
        ...
        // Verifica se o usuário existe, senão cria um temporário
        User tempUser = userService.findFullUserByEmail(email)
                .orElse(new Student("Usuário", email, new com.academiadev.model.BasicPlan()));
        supportTicketService.addTicket(tempUser, title, message);
        view.mostrarTicket(email, title, message);
    }
    ...
    private void alterarPlano() {
        ...
        if (view.confirmarAlteracaoPlano(alunoDto.getName(), planoAtual, novoPlanoNome)) {
            try {
                userService.changeSubscriptionPlan(email, novoPlano, currentUser instanceof com.academiadev.model.Admin ? (com.academiadev.model.Admin) currentUser : null);
