...
public class InitialData {
    ...
    public static void populate() {
        ...
        Course javaBasics = new Course("Introducao ao Java", "Aprenda os conceitos básicos de programação em Java.", "Profa. Ana Martins", 40, DifficultyLevel.BEGINNER, CourseStatus.ACTIVE);
        Course springBoot = new Course("Spring Boot Completo", "Crie aplicações REST modernas com Spring Boot e Hibernate.", "Prof. Carlos Silva", 60, DifficultyLevel.ADVANCED, CourseStatus.ACTIVE);
        Course pythonData = new Course("Python para Analise de Dados", "Trabalhe com Pandas, NumPy e visualização de dados.", "Dra. Júlia Fernandes", 50, DifficultyLevel.INTERMEDIATE, CourseStatus.ACTIVE);
        Course inactiveCourse = new Course("Historia dos Algoritmos", "Exploração de algoritmos clássicos que caíram em desuso.", "Prof. Roberto Antigo", 30, DifficultyLevel.BEGINNER, CourseStatus.INACTIVE);
        ...
        e3.setProgressPercent(85);
        DataStore.ENROLLMENTS.add(e3);

        Enrollment e4 = new Enrollment(bob, springBoot);
        e4.setProgressPercent(100);
        DataStore.ENROLLMENTS.add(e4);
        
        Enrollment e5 = new Enrollment(bob, pythonData);
        e5.setProgressPercent(15);
        DataStore.ENROLLMENTS.add(e5);

        Enrollment e6 = new Enrollment(charlie, javaBasics);
        e6.setProgressPercent(10);
        DataStore.ENROLLMENTS.add(e6);

        SupportTicket ticket1 = new SupportTicket(alice.getEmail(), "Problema no Pagamento", "Meu boleto da mensalidade não foi gerado.");
        SupportTicket ticket2 = new SupportTicket(bob.getEmail(), "Erro no Vídeo", "A aula 5 do curso de Spring Boot não carrega corretamente.");
